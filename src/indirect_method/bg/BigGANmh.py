...
def G_arch(ch=64, attention='64', ksize='333333', dilation='111111',n_channels=3):
  arch = {}
...
class Generator(nn.Module):
  def __init__(self, G_ch=64, dim_z=128, bottom_width=4, resolution=128,
               ...
               G_param='SN', norm_style='bn', n_channels = 3,
               **kwargs):
    ...
    self.output_layer = nn.Sequential(layers.bn(self.arch['out_channels'][-1],
                                                ...
                                                mybn=self.mybn),
                                    self.activation,
                                    self.which_conv(self.arch['out_channels'][-1], n_channels))
...
def D_arch(ch=64, attention='64',ksize='333333', dilation='111111', n_channels=3):
  arch = {}
  arch[256] = {'in_channels' :  [n_channels] + [ch*item for item in [1, 2, 4, 8, 8, 16]],
               'out_channels' : [item * ch for item in [1, 2, 4, 8, 8, 16, 16]],
               'downsample' : [True] * 6 + [False],
               'resolution' : [128, 64, 32, 16, 8, 4, 4 ],
               'attention' : {2**i: 2**i in [int(item) for item in attention.split('_')]
                              for i in range(2,8)}}
  arch[128] = {'in_channels' :  [n_channels] + [ch*item for item in [1, 2, 4, 8, 16]],
               'out_channels' : [item * ch for item in [1, 2, 4, 8, 16, 16]],
               'downsample' : [True] * 5 + [False],
               'resolution' : [64, 32, 16, 8, 4, 4],
               'attention' : {2**i: 2**i in [int(item) for item in attention.split('_')]
                              for i in range(2,8)}}
  arch[96]  = {'in_channels' :  [n_channels] + [ch*item for item in [1, 2, 4, 8, 16]],
               'out_channels' : [item * ch for item in [1, 2, 4, 8, 16, 16]],
               'downsample' : [True] * 5 + [False],
               'resolution' : [48, 24, 12, 6, 3, 3],
               'attention' : {3*2**i: 3*2**i in [int(item) for item in attention.split('_')]
                              for i in range(0,6)}}
  arch[64]  = {'in_channels' :  [n_channels] + [ch*item for item in [1, 2, 4, 8]],
  ...
  arch[48]  = {'in_channels' :  [n_channels] + [ch*item for item in [1, 2, 4, 8]],
               'out_channels' : [item * ch for item in [1, 2, 4, 8, 16]],
               'downsample' : [True] * 4 + [False],
               'resolution' : [48, 24, 12, 6, 6],
               'attention' : {3*2**i: 3*2**i in [int(item) for item in attention.split('_')]
                              for i in range(0,5)}}
  arch[32]  = {'in_channels' :  [n_channels] + [item * ch for item in [4, 4, 4]],
...
class Discriminator(nn.Module):
  ...
  def __init__(self, D_ch=64, D_wide=True, resolution=128,
               ...
               D_init='ortho', skip_init=False, D_param='SN', n_channels =3 ,**kwargs):
    ...
    self.fp16 = D_fp16
    # Architecture
    self.arch = D_arch(self.ch, self.attention, n_channels = n_channels)[resolution]
